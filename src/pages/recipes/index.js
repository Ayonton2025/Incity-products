...
const HealthcareBot = ({ toggleChat = () => {} }) => {
  ...
  const chatRef = useRef(null); // ✅ Hook declared at top
    ...
    return (
      <RootLayout>
        <div className="p-4 text-white">
          Please <a href="/auth/signin" className="text-blue-400">sign in</a> to use Recipe Bot.
        </div>
      </RootLayout>
    );
  }
  ...
  const handleChatInput = async () => {
    ...
    try {
      const response = await fetch("/api/recipes", {
        ...
        body: JSON.stringify({ message: messageInput, chatHistory, userId }),
      });
  ...
  async function handleClick() {
    ...
    router.push(`/places?query=${encodeURIComponent(key)}`);
  }
      ...
      <div ref={chatRef} className="w-full bg-black flex flex-col h-full p-4">
        ...
        <div className="flex flex-col gap-2 h-full overflow-y-auto">
          {chatHistory.map((message, index) => (
            <div
              key={index}
              className={`text-xl ${message.role === "user" ? "text-fuchsia-500" : "text-green-400"}`}
            >
              <ReactMarkdown>
                {`${message.role === "user" ? "You" : "Food Bot"}: ${message.text}`}
              </ReactMarkdown>
            </div>
          ))}
        ...
        <div className="flex items-center justify-center">
          ...
          />
          <button
            className="bg-blue-500 px-4 py-2 text-white rounded-md ml-2"
            disabled={!messageInput.trim() || loading}
            onClick={handleChatInput}
          >
            <MdOutlineChat size={24} />
          </button>
          <Button onClick={handleClick} className="text-red-500 underline ml-2">
            <HospitalIcon />
          </Button>
        </div>
      </div>
